...
class Species:
    ...
    def add_gc(self, conc, data): self.gc_data[conc] = data
    ...
    # Smooth a growth curve by taking 5-point medians
    def smooth(self, data):
        smoothed = [np.median(data[0:3]), np.median(data[0:4])] # First points
        for i in range(2, len(data) - 2): # For all middle points
            smoothed.append(np.median(data[i - 2:i + 2]))
        smoothed.append(np.median(data[-4:len(data)])) # Last points
        smoothed.append(np.median(data[-3:len(data)]))
        return smoothed
    ...
    def get_lines(self, data, mic, gc, gc_save):
        ...
        for conc in self.dilutions:
            ...
            conc_data = self.smooth(conc_data)
            # Guess logistic parameters
            k_guess = np.max(conc_data)
            
            # Add maximum OD as carrying capacity
            mic_data[conc] = k_guess
            
            # Add curve data if necessary
            if gc: self.add_gc(conc, conc_data)
    ...
    def graph_gcs(self, save):
        ...
        for conc in self.gc_data:
            ...
            ax.plot(self.xs, self.gc_data[conc], label='[' + 
...
def run(filename, sheet_name, filt, gc, mic, 
    ...
    # Average MIC curves; keys are antibiotics, values are dicts whose keys are
    # species names and values are the proportionally scaled MIC curve for that
    # combination
    scaled = {}
