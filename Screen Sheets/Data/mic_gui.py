...
def mics(file, antibiotic, strains, init_conc, plots, specs):
    ...
    for num in [0, 1]:
        ...
        for conc in dilutions:
            ...
            # Assume the sample showed no significant growth
                ...
                est_func, pcov = curve_fit(no_growth, x_vals, conc_data.values, 
                                           p0=[0, 0])
                conc_r[conc] = 0
                conc_k[conc] = k_guess
...
def gcs(file, antibiotic, strains, init_conc, plots):
    ...
    for num in [0, 1]:
        ...
        for conc in dilutions:
            ...
            try: # Implement curve fitting
                est_func, pcov = curve_fit(logistic, x_vals, conc_data.values, 
                                           p0=[r_guess, k_guess, x_0_guess])
                est_curve = logistic(x_vals, *est_func) # Obtain fitted data
            ...
            spec_ax.set(title="Growth of " + name + " in " + antibiotic + " Concentrations")
            
        plots.append((file, strains[num], fig, spec_ax)) # Add plot and name to list
    return plots
...
def check_match(word, itin):
    for elem in itin:
        for i in range(len(elem)):
            if word[i] != elem[i]: break;
        return True
    return False

def run(filename, filt, gc, mic, save_gc, save_mic, root):
    ...
    with open(filename, 'r', encoding='utf-8-sig') as files:
        for line in files:
            info = line.split()
            if check_match(info[0], filt):
                if mic: 
                    plots = mics(info[0], info[1], [info[2], info[3]], info[4],
                                 plots, species)
                if gc: 
                    gc_plots = gcs(info[0], info[1], [info[2], info[3]], 
                                   info[4], gc_plots)
    if mic:
        for plot in plots:
    ...
    if gc:
        for plot in gc_plots:
            plot[3].legend(loc=4)
            plot[3].autoscale()
            if save_gc: plot[2].savefig(plot[0][:-5] + "_" + plot[1] + "_GC.png")
    root.destroy()
...
def main():
    root = tk.Tk()
    root.title('My Title')
    frm = ttk.Frame(root, height=2)
    file_lbl = ttk.Label(root, text="Data List Filename:")
    file_ent = ttk.Entry(root, width = 50)
    focus_lbl = ttk.Label(root, text="Filter:")
    focus_txt = tk.Text(root, height = 3, width = 50)
    ...
    go = (lambda e=file_ent, t=focus_txt, g=gc_val, gs=gc_sv_val, m=mic_val, 
           ms=mic_sv_val, r=root: run(e.get(), t.get("1.0", tk.END).split(),
                                      g.get(), m.get(), gs.get(), ms.get(), r))
    ...
    file_ent.grid(row=1, column=1, padx = 5, pady = 5)
    focus_lbl.grid(row=2, column=0, sticky="e", padx = 5, pady = 5)
    focus_txt.grid(row=2, column=1, padx = 5, pady = 5)
    gc_chk.grid(row=3, column=0, padx = 20, pady = 5)
    mic_chk.grid(row=3, column=1, padx = 5, pady = 5)
    gc_sv_chk.grid(row=4, column=0, padx = 20, pady = 5)
    mic_sv_chk.grid(row=4, column=1, padx = 5, pady = 5)
    run_btn.grid(row=5, column=0, padx = 5, pady = 5)
